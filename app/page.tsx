...
export default function NovaTrade() {
  ...
  const sendMessage = () => {
    ...
    setTimeout(() => {
      ...
      if (lowerMsg.includes("nvda") || lowerMsg.includes("nvidia")) {
        response = "NVDA showing strong institutional buying. RSI bullish. Potential move to $148–$152 in next session. Set alert?";
      } else if (lowerMsg.includes("tsla") || lowerMsg.includes("tesla")) {
        response = "Tesla momentum positive near $350. Key resistance at $365. Shall I set a price alert?";
      } else if (lowerMsg.includes("gold") || lowerMsg.includes("gc")) {
        response = "Gold holding firm above $2650 with safe-haven flows. Strong technical support.";
      } else if (lowerMsg.includes("yen") || lowerMsg.includes("jpy")) {
        response = "USD/JPY testing 148 resistance. Yen remains structurally weak.";
      } else if (lowerMsg.includes("alert")) {
        response = "Alert configured successfully for your selected asset.";
        addAlert("NVDA", "NVIDIA", "Price above $148");
      } else {
        response = `Understood regarding "${userMsg}". What specific analysis or alert would you like?`;
      }
  ...
  const playSquawk = () => {
    if ('speechSynthesis' in window) {
      const utterance = new SpeechSynthesisUtterance("High impact market event detected. Gold breaking out on safe-haven flows.");
      utterance.rate = 1.05;
      utterance.pitch = 1.0;
  ...
  return (
    <div className="min-h-screen bg-[#0A0F1C] text-white flex flex-col">
      {/* Top Bar */}
      <header className="bg-[#111827] border-b border-white/10 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className="text-2xl font-bold text-[#00D4FF]">NovaTrade</div>
          <div className="text-xs px-3 py-1 bg-white/10 rounded-full">24/7 Global Intelligence</div>
        </div>
        
        <div className="flex items-center gap-6">
          <div className="text-emerald-400 text-sm">Live Markets • Open</div>
          <button onClick={playSquawk} className="bg-red-600 hover:bg-red-700 px-5 py-2 rounded-xl text-sm flex items-center gap-2">
            🔊 Test Squawk
          </button>
        </div>
      </header>
      ...
      <div className="flex flex-1 overflow-hidden">
          ...
          <div className="p-6">
            ...
            {menuItems.map(item => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-2xl mb-1 text-left transition-all ${activeTab === item.id ? 'bg-[#00D4FF] text-black font-medium' : 'hover:bg-white/5'}`}
              >
        ...
        <div className="md:hidden fixed top-4 left-4 z-50">
          <button 
            ...
            className="bg-[#1A2338] p-3 rounded-2xl text-2xl"
          >
            ☰
          </button>
        </div>
          ...
          <div className="max-w-5xl mx-auto">
            ...
            {activeTab === 'chat' && (
              <div className="space-y-6">
                ...
                <div className="bg-[#1A2338] rounded-3xl h-[60vh] flex flex-col p-6 overflow-hidden">
                  <div className="flex-1 overflow-auto space-y-6 pr-4" ref={messagesEndRef}>
                    {messages.map((msg, i) => (
                      <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[80%] px-6 py-4 rounded-3xl ${msg.role === 'user' ? 'bg-[#00D4FF] text-black' : 'bg-[#252F4A] text-gray-200'}`}>
                          {msg.content}
                        </div>
                      </div>
                    ))}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {[
                    ...
                    <div key={i} className="bg-[#1A2338] p-8 rounded-3xl">
                      <div className="text-2xl font-bold mb-2">{item.symbol}</div>
            ...
            {activeTab === 'calendar' && (
              <div>
                ...
                <div className="bg-[#1A2338] rounded-3xl p-8 space-y-6">
                      ...
                      <tbody>
                        {portfolio.map((item, i) => (
                          <tr key={i} className="border-b border-white/10 last:border-0">
                            ...
                            <td className="text-right py-5">${item.value}</td>
